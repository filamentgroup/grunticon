...
(function(window) {
	...
	*/
	...
	test( 'getIcons returns object', function(){
		expect(1);

		var icons = window.grunticon.getIcons("../files/icons.data.svg.css");
	...
	test( 'getIcons returns object with keys', function(){
		...
		var icons = window.grunticon.getIcons("icons.data.svg.css");
	...
	test( 'getIcons objects are accessible class with svg string', function(){
		...
		var icons = window.grunticon.getIcons("icons.data.svg.css");
		equal( icons['foo.icon-bear'], svg, "SVG contents should match" );
	});
