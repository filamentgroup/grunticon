...
(function( exports ){
	...
	Gfile.prototype.getCSSRules = function( stats, pngfolder, cssprefix, config ){
        ...
        var usecssdimensions = config.cssdimensions;
        var cssdimensions = "";
        var inlinestyledimensions = "";
        var dimensions = "width: " + stats.width + "; height: " + stats.height + ";";
		...
		var getPNGDataCSSRule = function( prefix, pngdatauri, relPngLocation, cssdimensions ){
			if (pngdatauri.length <= 32768) {
				// create png data URI
				return iconsel + " { background-image: url('" +  pngdatauri + "'); background-repeat: no-repeat;" + cssdimensions + "}";
			} else {
				return "/* Using an external URL reference because this image would have a data URI of " +
					...
					iconsel + " { background-image: url('" + relPngLocation.split( path.sep ).join( "/" ) + "'); background-repeat: no-repeat;" + cssdimensions + "}";

			}
		}; //getPNGDataCSSRule

        if (usecssdimensions){
            cssdimensions = " " + dimensions;
        } else {
            inlinestyledimensions = ' style="' + dimensions + '"';
        }

		res.pngcssrule = iconsel + " { background-image: url(" + pngfolder.split( path.sep ).join( "/" ) + this.filenamenoext + ".png" + "); background-repeat: no-repeat;" + cssdimensions + "}";
		res.htmlmarkup = '<pre><code>.' + prefix + ':</code></pre><div class="' + prefix + '"' + inlinestyledimensions + "></div><hr/>";
		res.datacssrule = iconsel + " { background-image: url('" + ( this.isSvg ? this.svgdatauri() : this.pngdatauri() ) + "'); background-repeat: no-repeat;" + cssdimensions + "}";
		res.pngdatacssrule = getPNGDataCSSRule( prefix , this.pngdatauri(), this.relPngLocation, cssdimensions );
		return res;
	};
