...
(function( exports ){
	...
	Gfile.prototype.getCSSRules = function( stats, pngfolder, cssprefix, config ){
		...
		var getPNGDataCSSRule = function( prefix , pngdatauri ){
			if (pngdatauri.length <= 32768) {
				...
				return "/* Using an external URL reference because this image would have a data URI of " +
					...
					iconsel + " { background-image: url('" + this.relPngLocation + "'); background-repeat: no-repeat; }";

			}
		}; //getPNGDataCSSRule

		res.pngcssrule = iconsel + " { background-image: url(" + pngfolder + this.filenamenoext + ".png" + "); background-repeat: no-repeat; }";
		...
		res.pngdatacssrule = getPNGDataCSSRule( prefix , this.pngdatauri() );
		return res;
	};
