...
(function( exports ){
	...
	var readDir = function( path ){
		...
		var createCSS = function( srcDir, o ){
			...
			.then( function( files , err ){
				...
				files = files.filter( function( file ){
					var stats = fs.lstatSync( path.resolve( path.join( srcDir , file ) ) );
				...
				var newFiles = GruntiFile.colorConfig( files , {
					inputDir: path.resolve( srcDir ),
					colors: JSON.parse( o.colors )
				});
				files.push.apply( files , newFiles );
				files.forEach( function( file, idx ){
					var gFile = new GruntiFile( file );
					var imgLoc = gFile.isSvg ? srcDir : path.resolve( path.join( srcDir , o.pngfolder ) );
