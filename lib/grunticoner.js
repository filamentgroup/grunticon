...
(function(exports) {
	...
	exports.createCSSRules = function( gFile , o ){
		...
		// Temp fix for repetitious no-repeat (wat).
		var extraCSS = '';
		...
		var getPNGDataCSSRule = function( prefix , pngimgstring ){
			if (pngimgstring.length <= 32768) {
				...
				return "/* Using an external URL reference because this image would have a data URI of " +
					...
					iconsel + " { background-image: url(" + o.pngout + filenamenoext + ".png" + ");"+extraCSS+"}";
			}
		}; //getPNGDataCSSRule
		if( isSvg ) {
		...
		res.pngcssrule = iconsel + " { background-image: url(" + o.pngout + filenamenoext + ".png" + ");"+extraCSS+"}";
	...
	exports.render = function( gFile , o) {
		...
		console.log('>> '+filename+' ('+gFile.width+' x '+gFile.height+') => '+pngDest + pngName);
	...
	exports.writeCSS = function( dataarr , o ){
		...
		basepath = o.cssbasepath + o.outputdir;

		asyncCSS = [
			fs.read( o.asyncCSSpath ),
			'\ngrunticon([',
		...
		htmldoc = createHTMLDoc(htmlpreviewbody, asyncCSS, o);
